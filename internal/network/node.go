...
import (
	...
	p2p "github.com/sohelahmedjoni/pqhbevs_hac/internal/blockchain"
...
type Node struct {
	...
	*BlockProtocol // ping protocol impl
...
func NewNode(host host.Host, done chan bool) *Node {
	node := &Node{Host: host}
	node.BlockProtocol = NewBlockProtocol(node, done)
...
func (n *Node) NewMessageData(messageId string, gossip bool, msg *p2p.MessageData) *p2p.MessageData {
	// Add protobuf binary data for message author public key
...
func (n *Node) NewRespData(messageId string, gossip bool, block *p2p.Block, binarydata *p2p.BinaryData) *p2p.MessageData {
	// Add protobuf binary data for message author public key
	// this is useful for authenticating messages forwarded by a node authored by another node
	nodePubKey, err := crypto.MarshalPublicKey(n.Peerstore().PubKey(n.ID()))
...
// helper method - generate message data shared between all node's p2p protocols
// messageId: unique for requests, copied from request for responses
func (n *Node) NewMessageLiteData(messageId string, gossip bool, block *p2p.LiteBlock) *p2p.MessageData {
...
func (n *Node) sendProtoMessage(ctx context.Context, id peer.ID, p protocol.ID, data proto.Message) bool {
